(function(){
	function Slide(wraperBox,next,pre,ulcontent,opt){
		...
		this.Extend(this, this.option(opt));
		this.t = null;
	...
	Slide.prototype.option=function(options){ 
			...
			return this.Extend(this.option,options||{})
		};
	// 调用图片轮播函数
	Slide.prototype.Extend=function(destination, source){
		for(pro in source)
		{
	...
	Slide.prototype.listMouseenter = function() {
		...
		_this.ulList.delegate('>li', 'mouseenter', function() {
			...
			W(this).addClass('on');
			//找到当前移入的li的index
			var nowIndex=_this.findLiIndex(allList, 'on');
			_this.move(nowIndex);
		});
	}
